...
describe Lexer do
  ...
  describe 'Number Finder' do
    it 'should find integers' do
      @lexer.from '12345'
      expect( @lexer.next ).to eq Token.new( :integer, 12_345 )
    end

    it 'should find negative values' do
      @lexer.from '-12345'
      expect( @lexer.peek_next ).to eq Token.new( :integer, -12_345 )
      expect( @lexer.next ).to eq Token.new( :integer, -12_345 )
    end
  ...
  describe '.peek_next' do
    it 'should return the next token, and leave it available for .next' do
      @lexer.from '"dq string"'
